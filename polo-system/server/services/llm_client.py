...
class EasyLLMClient:
    ...
    def __init__(self, base_url: str = "http://localhost:5003", default_timeout: int = 600, max_retries: int = 2):
    ...
    def generate(self, text: str, max_length: int = 512, temperature: float = 0.7) -> Optional[dict]:
        """논문 텍스트를 /generate로 보내 JSON 변환 결과를 받는다 (재시도/타임아웃 확장)"""
        payload = {"text": text}
        ...
        for attempt in range(1, self.max_retries + 2):  # initial try + retries
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"텍스트 생성 실패({attempt}/{self.max_retries+1}): {response.status_code} - {response.text}")
                    last_error = RuntimeError(response.text)
            except Exception as e:
                last_error = e
                logger.error(f"텍스트 생성 요청 실패({attempt}/{self.max_retries+1}): {e}")
            # backoff
